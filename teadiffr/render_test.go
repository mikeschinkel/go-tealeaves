...
func TestRenderFileDiffs_SingleFile(t *testing.T) {
	...
	// First line should be file header
	// Remaining lines: block header + 1 context before + 2 added + 1 context after = 5
	// Total: 1 (header) + 1 (block header) + 1 (context before) + 2 (added) + 1 (context after) = 6
	if len(lines) != 6 {
...
func TestRenderFileDiffs_MultipleFiles(t *testing.T) {
	...
	// 2 file headers + 1 blank separator line between them = 3
	if len(lines) != 3 {
...
func TestRenderFileDiffs_Truncation(t *testing.T) {
	...
	// header + block header + 1 deleted line + truncation marker = 4
	if len(lines) != 4 {
...
func TestNewTUIRenderer_CustomArgs(t *testing.T) {
	...
	// Custom color should be set, others should be defaults
